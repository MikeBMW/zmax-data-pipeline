...
def _enqueue_detect(img_path, detect_type):
    """投递检测任务, 立即返回"""
    with _detect_lock:
        _detect_queue.append((img_path, detect_type))
...
def capture_detect_api():
    try:
        ...
        # 【异步】投递检测队列, 立即返回
        _enqueue_detect(img_path, detect_type)
...
if __name__ == "__main__":
    print("表面检测相机程序启动(优化版v3)，端口10083，housing表面模型，相机常驻模式")
    # 启动时预初始化相机 (可选, 加快首个请求)
    try:
